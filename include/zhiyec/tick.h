 ...
 * @brief 获取当前Tick
 */
#define tick_get_current() ((tick_t)kernel_ticks)

/**
 * @brief 毫秒转Tick
 */
#define tick_from_msec(ms) ((tick_t)(((ms) * SYSTICK_RATE_HZ + (1000 - 1)) / 1000))

/**
 * @brief Tick转毫秒
 */
#define tick_to_msec(ticks) ((unsigned int)(((ticks) * 1000UL + (SYSTICK_RATE_HZ - 1)) / SYSTICK_RATE_HZ))
